         ...
         unsigned char *output)
{
 ...
 sm4_setkey( sk, key );
 if( dec_en==SM4_ENC){           //encryption
	  for(i=0;i<block_num;i++)
	  {
		  sm4_enc_128bit(sk,input+i*16,output+i*16);
	  }
 }
 else{
	 for(i=0;i<block_num;i++)
	 {
		 sm4_dec_128bit(sk,input+i*16,output+i*16);
	 }
...
int sm4_cfb( unsigned char key[16],
                    int dec_en,
                    int flag,
                    unsigned char iv[16],
                    unsigned char *input,
 ...
 * SM4 ofb mode
 */
int sm4_ofb( unsigned char key[16],
                    int dec_en,
                    int flag,
                    unsigned char iv[16],
                    unsigned char *input,
...
int sm4_ctr( unsigned char key[16],
                    int dec_en,
                    int flag,
 ...
 * SM4 xts mode
 */
int sm4_xts( unsigned char key2[16],
                    ...
                    unsigned char *output )
{
    ...
    if( dec_en == 0 )   //encryption
    {
            ...
            //enc C get P
             sm4_enc_128bit(sk,C_temp,P_temp);
            //T xor P
            for( i = 0; i < 16; i++ )
    ...
    else //dencryption
    {
            ...
            for( i = 0; i < 16; i++ )
                C_temp[i] = (unsigned char)( pbIV[i] ^ input[i] );
            //dec C get P
             sm4_dec_128bit(sk,C_temp,P_temp);
            //T xor P
            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( pbIV[i] ^ P_temp[i] );
            //LSFR
                    ...
                    unsigned char *output )
{
       /* switch(mode){
                case 0:
                	 sm4_ecb(key1,dec_en,flag,input,output);
                	 break;
                case 1:
                	return sm4_cbc(key1,dec_en,flag,iv,input,output);
                case 2:
                	return sm4_cfb(key1,dec_en,flag,iv,input,output);
                case 3:
                	return sm4_ofb(key1,dec_en,flag,iv,input,output);
                case 4:
                	return sm4_ctr(key1,dec_en,flag,iv,input,output);
                case 5:
                	return sm4_xts(key1,key2,dec_en,flag,iv,input,output);
                default:
                	return sm4_xts(key1,key2,dec_en,flag,iv,input,output);
