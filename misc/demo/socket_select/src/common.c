...
void sdf_rev_cmd(int sockfd,char* data,int len)
{
	...
	if(rev_xmit_protocol->len<sizeof(xmit_protocol))
	{
		printfS("rev_xmit_protocol->len<sizeof(xmit_protocol)\n");
		//send(sockfd, rev_xmit_protocol, rev_xmit_protocol->len, 0);
		//printfS("send success\n");
		free_point(rev_xmit_protocol);
		return;
	}
	if((rev_xmit_protocol->len!=len)&&(rev_xmit_protocol->len>sizeof(xmit_protocol)))
	{
		printfS("rev_xmit_protocol->len error\n");
		send(sockfd, rev_xmit_protocol, sizeof(packet_head), 0);
		printfS("send success\n");
		free_point(rev_xmit_protocol);
		return;
	}
	...
	printfS("send->len=%d\n",rev_xmit_protocol->len);
	send(sockfd, (unsigned char *)rev_xmit_protocol, rev_xmit_protocol->len, 0);
	PrintBuf((unsigned char *)rev_xmit_protocol,rev_xmit_protocol->len);
	printfS("send success\n");
	free_point(rev_xmit_protocol);
}
