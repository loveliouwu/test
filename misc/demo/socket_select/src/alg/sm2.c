...
*/
	...
	if ((mr_compare(r, n) >= 0)  || (r->len == 0))
		goto exit_sm2_verify;

	if ((mr_compare(s, n) >= 0) || (s->len == 0))
		goto exit_sm2_verify;
...
int sm2_encrypt(unsigned char *msg,int msglen, unsigned char *wx,int wxlen, unsigned char *wy,int wylen, unsigned char *outmsg, int *outlen)
{
/*
...
[输出] outlen: 加密结果Len，比输入数据多96字节！outmsg+96，C1（64字节）和C3（32字节）保留前导0x00
...
int sm2_decrypt(unsigned char *msg,int msglen, unsigned char *privkey, int privkeylen, unsigned char *outmsg, int *outlen)
{
/*
...
[输出] outlen: 解密结果Len，比输入数据少96字节, msglen-96！
